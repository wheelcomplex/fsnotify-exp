...
const EPOLL_DEFAULT_MAX_EVENTS = 16
...
type Watcher struct {
	...
	eventSize int               // epoll_create size
}

// NewWatcher establishes a new watcher with the underlying OS and begins waiting for events.
func NewWatcher(size int) (*Watcher, error) {
	fd, errno := syscall.InotifyInit()
	if fd == -1 {
		return nil, os.NewSyscallError("inotify_init", errno)
	}
	if size <= 0 {
		size = EPOLL_DEFAULT_MAX_EVENTS
	}
	w := &Watcher{
		fd:        fd,
	...
	// syscall.EPOLLET buggy, should be 0x80000000
	// ET
	//event := &syscall.EpollEvent{syscall.EPOLLIN | 0x80000000, int32(w.fd), 0}
	// LT
	...
	go func() {
		...
		// so epollEvents exit and Events channel closed
	}()
...
// IsWatched return true if name already watched
...
func (w *Watcher) epollEvents(epfd int, donePipe *os.File) {
	...
	events := make([]syscall.EpollEvent, w.eventSize)
	...
	for {
		...
		for i := 0; i < nevents; i++ {
			if events[i].Fd == doneFd {
				...
				println("events[i].Fd != int32(w.fd)", i, events[i].Fd, w.fd)
				continue
			}
